...
print_step "Syncing global extension symlinks (~/.omp/agent/extensions)"
ext_dir="$HOME/.omp/agent/extensions"
ext_pairs=(
	"antigravity-adapter/antigravity.bundle.js|$repo_dir/packages/antigravity-adapter/dist/antigravity.bundle.js"
	"pi-minimizer-gain/gaing.bundle.js|$repo_dir/packages/pi-minimizer-gain/dist/gaing.bundle.js"
	"pi-observer/observer.bundle.js|$repo_dir/packages/pi-observer/dist/observer.bundle.js"
	"pi-actor-swarm/swarm.bundle.js|$repo_dir/packages/pi-actor-swarm/dist/swarm.bundle.js"
	"pi-omnidelegate/omnidelegate.bundle.js|$repo_dir/packages/pi-omnidelegate/dist/omnidelegate.bundle.js"
	"pi-software-factory/factory.bundle.js|$repo_dir/packages/pi-software-factory/dist/factory.bundle.js"
	"swarm-extension/extension.bundle.js|$repo_dir/packages/swarm-extension/dist/extension.bundle.js"
	"profile-manager/index.js|$repo_dir/.omp/extensions/profile-manager/dist/index.js"
)
for pair in "${ext_pairs[@]}"; do
	rel="${pair%%|*}"
	tgt="${pair##*|}"
	if [[ ! -f "$tgt" ]]; then
		printf 'error: extension build output missing: %s\n' "$tgt" >&2
		exit 1
	fi
	mkdir -p "$ext_dir/${rel%/*}"
	ln -sfn "$tgt" "$ext_dir/$rel"
	printf '  %s\n' "$rel"
done
if [[ ! -f "$HOME/.omp/agent/settings.json" ]]; then
	printf 'warning: ~/.omp/agent/settings.json missing — global extensions/profiles will not load.\n' >&2
	printf '         Restore it (extensions[] + modelProfiles) for omp/lex to work outside this repo.\n' >&2
fi
