// @bun
// .omp/extensions/pane-coms/index.ts
import * as fs from "fs/promises";
import * as net from "net";
import * as os from "os";
import * as path from "path";
var state = { inbox: [], waiters: [] };
var ROOT = path.join(os.homedir(), ".omp", "pane-coms");
var REGISTRY_FILE = path.join(ROOT, "registry.json");
function paneComsExtension(pi) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      if (!message)
        return { content: [{ type: "text", text: "No pane-coms message received before timeout." }] };
...
async function identifyCmux(pi) {
  try {
...
function handleSocket(pi, socket) {
  let data = "";
...
async function peersInWorkspace() {
  const registry = await readRegistry();
...
async function readRegistry() {
  try {
    const parsed = JSON.parse(await Bun.file(REGISTRY_FILE).text());
    return parsed.filter((entry) => entry.socketPath && Date.now() - entry.updatedAt < 86400000);
  } catch {
    return [];
  }
}
...
function connectAndWrite(socketPath, payload) {
  const { promise, resolve, reject } = Promise.withResolvers();
...
function formatRegistry(entries, current) {
  const lines = ["Pane-coms peers:"];
  if (current)
    lines.push(`* ${current.name} ${current.surfaceRef} cwd=${current.cwd}`);
  for (const entry of entries)
    lines.push(`  ${entry.name} ${entry.surfaceRef} cwd=${entry.cwd}`);
  if (lines.length === 1)
    lines.push("  (none)");
  return lines.join(`
`);
}
function formatStatus(entry, inbox) {
