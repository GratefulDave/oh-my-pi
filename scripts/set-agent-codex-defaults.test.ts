...
import { checkAgentConfigFiles, normalizeAgentConfigFiles, normalizeAgentDefaults } from "./set-agent-codex-defaults";
...
describe("agent Codex defaults", () => {
	...
	it("reports needed defaults without changing the config", async () => {
		const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "agent-codex-defaults-"));
		tempDirs.push(homeDir);
		const canonical = path.join(homeDir, ".omp", "agent", "config.yml");
		await Bun.write(canonical, YAML.stringify({ task: { agentModelOverrides: {} } }, null, 2));
		...
		expect(await checkAgentConfigFiles(homeDir)).toEqual([await fs.realpath(canonical)]);
		expect(await Bun.file(canonical).text()).toBe(original);
	});
});
