...
async function inspectAgentConfigFiles(homeDir: string, applyChanges: boolean): Promise<string[]> {
	...
	for (const configPath of await configPaths(homeDir)) {
		...
		if (applyChanges) await Bun.write(realPath, YAML.stringify(parsed, null, 2));
...
export async function normalizeAgentConfigFiles(homeDir: string): Promise<string[]> {
	return inspectAgentConfigFiles(homeDir, true);
}

export async function checkAgentConfigFiles(homeDir: string): Promise<string[]> {
	return inspectAgentConfigFiles(homeDir, false);
}
...
if (import.meta.main) {
	...
	process.stdout.write(
		check
			...
			: `Agent Codex defaults applied to ${changed.length} config file(s).\n`,
	);
