 ...
 * active I/O watchers (stdin, child process pipes, etc.). This causes 100%
 * CPU usage on idle interactive sessions, including:
...
/**
 * Sleep for at least `ms` milliseconds. Works around a Bun quirk where
 * `scheduler.wait` may return prematurely (which can happen when napi
