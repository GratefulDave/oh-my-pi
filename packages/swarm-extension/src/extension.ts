...
export default function swarmExtension(pi: ExtensionAPI): void {
	...
	pi.registerCommand("swarm", {
		...
		getArgumentCompletions: prefix => {
			...
			if (!prefix) return subcommands.map(command => ({ label: command, value: command }));
			return subcommands
...
async function handleRun(yamlPath: string, ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<void> {
	...
	const waveDesc = waves.map((wave, index) => `wave ${index + 1}: [${wave.join(", ")}]`).join("; ");
	...
	pi.sendMessage(
		{
			...
			content: [{ type: "text", text: buildSummaryMessage(def, result, stateTracker, workspace) }],
			display: true,
			details: {
				swarmName: def.name,
