...
// packages/swarm-extension/src/swarm/render.ts
...
// packages/swarm-extension/src/swarm/state.ts
...
`)}`,
	...
	const workspace = path3.isAbsolute(def.workspace)
		? def.workspace
		: path3.resolve(path3.dirname(resolvedPath), def.workspace);
	...
	const elapsed = stateTracker.state.completedAt
		? formatDuration2(stateTracker.state.completedAt - stateTracker.state.startedAt)
		: "unknown";
	const summaryParts = [
...
function buildSummaryMessage(def, result, stateTracker, workspace) {
	...
	for (const [name, agent] of Object.entries(stateTracker.state.agents)) {
		const duration =
			agent.startedAt && agent.completedAt ? formatDuration2(agent.completedAt - agent.startedAt) : "n/a";
...
export { swarmExtension as default };
