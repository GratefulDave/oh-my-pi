...
export class MismatchError extends Error {
	...
	static rejectionHeader(details: MismatchDetails): string[] {
		...
		if (details.hashRecognized === false) {
			return [
				`Edit rejected${pathText}: hash ${HL_FILE_HASH_SEP}${details.expectedFileHash} is not from this session for this file.`,
				`never invent the tag; re-read the file and use the fresh ${HL_FILE_PREFIX}path${HL_FILE_HASH_SEP}hash header. The current file hashes to ${HL_FILE_HASH_SEP}${details.actualFileHash}.`,
			];
		}
		return [
