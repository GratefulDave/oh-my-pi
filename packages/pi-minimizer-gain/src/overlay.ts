...
const SCOPES = ["Active", "Current", "All"] as const;
...
export class MinimizerGainOverlayComponent {
	...
	#getActiveContext(): MinimizerGainContext {
		if (this.#activeScopeIndex === 0) return this.#dualContext.active;
	...
	render(width: number): string[] {
		...
		lines.push(
			clean(
				`${this.#heading("Minimizer Gain")} ${this.#formatTab("Active", activeScope === "Active")} ${this.#dim("|")} ${this.#formatTab("Current", activeScope === "Current")} ${this.#dim("|")} ${this.#formatTab("All", activeScope === "All")}`,
				width,
			),
		);
		lines.push(
		...
		const scopeText =
			activeScope === "Active"
				? `active session · ${shortenPath(context.cwd ?? process.cwd())}`
				: context.all
					? "all repos"
					: shortenPath(context.cwd ?? process.cwd());
		lines.push(clean(this.#muted(`Scope: ${scopeText}`), width));
		lines.push("");
		...
		if (activeTab === "Gain") {
			const scopeLabel =
				activeScope === "Active" ? "Active Session" : context.all ? "Global Scope" : "Current Scope";
