...
## [Unreleased]
### Added
- Added embedded session observer cards for async background jobs, including bash jobs and task jobs, so their lifecycle/progress metadata can appear beside task/plugin subagent cards while `job` remains the polling/cancel control surface.
