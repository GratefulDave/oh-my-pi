...
describe("Python tool bridge HTTP server", () => {
	...
	it("surfaces tool errors as ok=false with the error message", async () => {
		...
		const info = await ensurePyToolBridge();
		const unregister = registerPyToolBridge("err-session", "err-run", { toolSession: session });
		try {
			const res = await call(info, { session: "err-session", run: "err-run", name: "boom", args: {} });
			expect(res.status).toBe(200);
			const body = await res.json();
	...
	it("rejects requests with a bad bearer token", async () => {
		const info = await ensurePyToolBridge();
		const res = await call(info, { session: "anything", run: "run", name: "read", args: {} }, { token: "wrong" });
	...
	it("invokes emitStatus alongside the tool result", async () => {
		...
		const unregister = registerPyToolBridge("status-session", "status-run", {
			toolSession: session,
