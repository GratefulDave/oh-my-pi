...
describe("YieldQueue", () => {
	...
	test("MCP-style notifications batch and dedupe by server and URI", async () => {
		type McpNotificationEntry = { serverName: string; uri: string };
		const harness = createHarness(true);
		harness.queue.register<McpNotificationEntry>("mcp-notification", {
			build: entries => {
				...
				return userMessage(resources.map(entry => `${entry.serverName}:${entry.uri}`).join("\n"));
			},
		});

		harness.queue.enqueue("mcp-notification", { serverName: "a", uri: "file://one" });
		harness.queue.enqueue("mcp-notification", { serverName: "a", uri: "file://one" });
		harness.queue.enqueue("mcp-notification", { serverName: "b", uri: "file://one" });
		await harness.queue.flush("streaming");

		expect(harness.streamingMessages.map(messageText)).toEqual(["a:file://one\nb:file://one"]);
	});
});
