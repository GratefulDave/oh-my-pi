/**
 * Markdown structure awareness for magic-keyword affordances
	...
	while (lineStart <= n) {
		...
		if (fenceChar) {
			...
			if (
				...
				line.slice((open[1]?.length ?? 0) + open[2].length).trim() === ""
			) {
				fenceChar = "";
				fenceLen = 0;
			}
			...
			const marker = open[2];
			const ch = marker?.[0];
			if (marker && ch && !(ch === "`" && line.slice((open[1]?.length ?? 0) + marker.length).includes("`"))) {
				fenceChar = ch;
