...
export async function appendBashMinimizerGainRecord(input: BashMinimizerGainInput): Promise<void> {
	...
	const observedInputBytes =
		kind === "missed" && input.inputBytes <= 0 ? Buffer.byteLength(input.command) : input.inputBytes;
	const observedOutputBytes = kind === "missed" && input.outputBytes <= 0 ? observedInputBytes : input.outputBytes;
	if (kind === "missed" && observedInputBytes <= 0) return;

	const recordsPath = getBashMinimizerGainPath(input.agentDir);
	const record = {
		timestamp: new Date().toISOString(),
		...(input.cwd ? { cwd: await resolveCwd(input.cwd) } : {}),
