...
- SuperGrok (`xai-oauth` only): force `parallel_tool_calls: true` (xAI defaults this off) and ingest Completions-shaped `tool_calls` on a Responses stream, including streams that never send a Responses terminal frame, so every Grok function call actually executes.
