...
export type ObserverThemeColor =
	| "accent"
	| "border"
	| "borderAccent"
	| "success"
...
export interface RenderTheme {
	fg(color: ObserverThemeColor, text: string): string;
...
const RESET = "\x1b[0m";
const SUCCESS_FG = "\x1b[32m";
const ERROR_FG = "\x1b[31m";
const ACCENT_TINT = "\x1b[46;30m";
const SUCCESS_TINT = "\x1b[42;30m";
const ERROR_TINT = "\x1b[41;37m";
const MUTED_TINT = "\x1b[48;5;238m\x1b[37m";
const BORDER_ACCENT_TINT = "\x1b[48;5;24m\x1b[37m";
...
export function padTintedLine(text: string, width: number, tint = ACCENT_TINT): string {
	...
	#renderAgent(
		...
		if (agent.status === "running") {
			...
			return options.compact
				...
				: [
						padTintedLine(header, options.width, selected ? BORDER_ACCENT_TINT : ACCENT_TINT),
		...
		if (agent.status === "completed") {
			...
			return [
				options.compact
					? color(SUCCESS_FG, header)
					: padTintedLine(header, options.width, selected ? BORDER_ACCENT_TINT : SUCCESS_TINT),
			];
		}
		if (agent.status === "failed" || agent.status === "aborted") {
			...
			return state
				? [
						options.compact
							? color(ERROR_FG, header)
							: padTintedLine(header, options.width, selected ? BORDER_ACCENT_TINT : ERROR_TINT),
		...
		return [
			options.compact
				? `◇ [${role}] ${label}`
				: padTintedLine(`◇ [${role}] ${label}`, options.width, selected ? BORDER_ACCENT_TINT : MUTED_TINT),
		];
	}
}
