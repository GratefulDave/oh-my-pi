...
import type { ExtensionAPI, SubagentLifecyclePayload, SubagentProgressPayload } from "@oh-my-pi/pi-coding-agent";
...
export default function observer(pi: ExtensionAPI): void {
	...
	pi.on("after_provider_response", (event, ctx) => {
		...
		if (usage) {
			const modelId = event.response.model ?? ctx.getModel?.()?.id ?? "unknown";
