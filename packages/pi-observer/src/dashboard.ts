...
function colorForStatus(status: ObserverNode["status"]): ObserverThemeColor {
	switch (status) {
		case "completed":
			return "success";
		case "failed":
...
export class ObserverDashboard {
	...
	#renderPanel(hierarchy: ObserverHierarchy, width: number, height: number): string[] {
		...
		const rightLines = rightNode
			...
			: [this.theme.fg("dim", "No hierarchy nodes observed yet")];
	...
	#renderNodeDetail(node: ObserverNode, width: number, height: number): string[] {
		...
		if (node.children.length > 0) lines.push(this.theme.fg("dim", `${node.children.length} children · ↵ drill`));
		else lines.push(this.theme.fg("dim", expanded ? "Expanded · ↵ collapse" : "Leaf · ↵ expand"));
		if (node.metrics) {
		...
		lines.push(
			this.theme.fg("dim", this.#activePane === "detail" ? "↑↓ scroll detail · tab tree" : "tab detail · ↑↓ select"),
		);
		lines.push("");
		...
		if (scroll > 0 && viewport > 1 && content.length > 0)
			content[scroll] = this.theme.fg("dim", `↑ ${scroll} earlier lines`);
		if (viewport > 1 && scroll + viewport < content.length && content.length > 0) {
			const last = Math.min(content.length - 1, scroll + viewport - 1);
			content[last] = this.theme.fg("dim", `↓ ${content.length - scroll - viewport} later lines`);
		}
