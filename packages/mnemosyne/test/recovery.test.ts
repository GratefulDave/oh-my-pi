...
function makeTempDir(): string {
	const dir = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-recovery-"));
	tempDirs.push(dir);
	return dir;
}
...
function writeCorruptSqliteBackup(path: string): void {
	fs.writeFileSync(path, zlib.gzipSync(Buffer.concat([SQLITE_HEADER, Buffer.from("corrupt backup payload")])));
}
...
describe("SQLite recovery helpers", () => {
	it("creates a compressed backup with metadata", () => {
		...
		const dbPath = nodePath.join(dir, "mnemosyne.db");
		const backupDir = nodePath.join(dir, "backups");
		...
		expect(fs.existsSync(backup.metadata_path)).toBe(true);
		expect(backup.original_size).toBe(fs.statSync(dbPath).size);
		expect(backup.backup_size).toBe(fs.statSync(backup.backup_path).size);
		expect(backup.compressed).toBe(true);
		expect(
			Buffer.from(zlib.gunzipSync(fs.readFileSync(backup.backup_path)))
				.subarray(0, 16)
				.toString("binary"),
	...
	it("creates distinct backup files when called twice in the same second", () => {
		...
		expect(fs.existsSync(first.backup_path)).toBe(true);
		expect(fs.existsSync(second.backup_path)).toBe(true);
		expect(fs.existsSync(first.metadata_path)).toBe(true);
		expect(fs.existsSync(second.metadata_path)).toBe(true);
	});
	...
	it("restores a backup to a new path", () => {
		const dir = makeTempDir();
		const dbPath = nodePath.join(dir, "mnemosyne.db");
		const restoredPath = nodePath.join(dir, "restored.db");
		createSqliteDb(dbPath);
		const backup = createBackup(dbPath, nodePath.join(dir, "backups"));
	...
	it("keeps the current WAL database untouched when a staged restore fails integrity", () => {
		...
		const badBackup = nodePath.join(backupDir, "mnemosyne_backup_20260530_120000.db.gz");
	...
	it("leaves the original database intact when emergency restore exhausts corrupt backups", () => {
		...
		createSqliteDb(dbPath);
		fs.mkdirSync(backupDir, { recursive: true });
		writeCorruptSqliteBackup(nodePath.join(backupDir, "mnemosyne_backup_20260530_120000.db.gz"));
