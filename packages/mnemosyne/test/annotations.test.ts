...
function tempDb(): string {
	const dir = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-annotations-"));
	cleanup.push(dir);
	return nodePath.join(dir, "annotations.db");
}

afterEach(() => {
	while (cleanup.length > 0) {
		const path = cleanup.pop();
		if (path) fs.rmSync(path, { recursive: true, force: true });
	}
});
