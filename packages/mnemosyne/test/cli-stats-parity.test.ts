...
beforeEach(() => {
	root = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-ts-cli-stats-parity-"));
...
describe("CLI stats parity", () => {
	it("prints real working, episodic, triple, bank, and database counts", () => {
		const dbPath = nodePath.join(root, "mnemosyne.db");
	...
	it("stats commands read the configured data directory, not the default home path", async () => {
		const customDataDir = nodePath.join(root, "custom-data");
		const io = capture();
		expect(cmdRemember(["stats data dir probe"], io.context(customDataDir))).toBe(0);
		expect(fs.existsSync(nodePath.join(customDataDir, "mnemosyne.db"))).toBe(true);
		...
		expect(statsIo.stdout).toContain(nodePath.join(customDataDir, "mnemosyne.db"));
	});
});
...
describe("mnemosyne-stats diagnostic behavior parity", () => {
	...
	it("diagnostics initialize missing databases gracefully and report zero counts", () => {
		const dbPath = nodePath.join(root, "empty", "mnemosyne.db");
		fs.mkdirSync(nodePath.join(root, "empty"), { recursive: true });
		const result = runDiagnostics({ dbPath, dataDir: nodePath.join(root, "empty") });
		expect(result.database).toBe(dbPath);
