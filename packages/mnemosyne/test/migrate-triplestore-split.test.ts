...
function tempDb(): string {
	const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-ts-e6-"));
	roots.push(root);
	return nodePath.join(root, "triples.db");
}
...
describe("E6 triplestore split migration", () => {
	...
	it("reports dry-run counts without writes and writes backup only when requested", () => {
		...
		expect(fs.existsSync(`${dbPath}.pre_e6_backup`)).toBe(false);

		expect(migrate({ dbPath, backup: true, logFn: () => undefined })).toBe(2);
		expect(fs.existsSync(`${dbPath}.pre_e6_backup`)).toBe(true);
		fs.writeFileSync(`${dbPath}.pre_e6_backup`, "sentinel");
		seedRows(dbPath, [["mem-2", "mentions", "Carol", "extraction", 0.8]]);
		expect(migrate({ dbPath, backup: true, logFn: () => undefined })).toBe(1);
		expect(fs.readFileSync(`${dbPath}.pre_e6_backup`, "utf8")).toBe("sentinel");
	});
