...
describe("BankManager", () => {
	it("creates, lists, renames, stats, and deletes isolated bank directories", () => {
		const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-banks-"));
		try {
			...
			expect(manager.getBankDbPath("default")).toBe(nodePath.join(root, "mnemosyne.db"));
			expect(manager.getBankDbPath("work")).toBe(nodePath.join(root, "banks", "work", "mnemosyne.db"));
			expect(manager.getBankStats("work").db_size_bytes).toBeGreaterThanOrEqual(0);
			const renamed = manager.renameBank("work", "project_a");
			expect(renamed).toBe(nodePath.join(root, "banks", "project_a", "mnemosyne.db"));
	...
	it("validates names and protects default deletion", () => {
		...
		try {
			...
			expect(manager.getBankDbPath("")).toBe(nodePath.join(root, "mnemosyne.db"));
