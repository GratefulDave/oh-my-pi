...
function tempRoot(): string {
	const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-ts-triples-"));
	roots.push(root);
	return root;
}
...
describe("TripleStore default data-directory handling", () => {
	it("keeps triples.db beside the configured Mnemosyne data directory", () => {
		...
		const dataDir = nodePath.join(root, "configured-data");
		process.env.HOME = home;
		process.env.MNEMOSYNE_DATA_DIR = dataDir;

		const store = new TripleStore();
		try {
			expect(store.dbPath).toBe(nodePath.join(dataDir, "triples.db"));
			expect(defaultTripleDbPath()).toBe(nodePath.join(dataDir, "triples.db"));
			expect(fs.existsSync(nodePath.join(dataDir, "triples.db"))).toBe(true);
			expect(fs.existsSync(nodePath.join(home, ".hermes", "mnemosyne", "data", "triples.db"))).toBe(false);
		} finally {
			store.close();
		}
	});
	...
	it("copies an existing legacy triples database into MNEMOSYNE_DATA_DIR", () => {
		...
		const home = nodePath.join(root, "home");
		const dataDir = nodePath.join(root, "configured-data");
		const legacyDb = nodePath.join(home, ".hermes", "mnemosyne", "data", "triples.db");
		process.env.HOME = home;
		process.env.MNEMOSYNE_DATA_DIR = dataDir;
		...
		expect(fs.existsSync(nodePath.join(dataDir, "triples.db"))).toBe(false);

		const migrated = new TripleStore();
		try {
			expect(migrated.dbPath).toBe(nodePath.join(dataDir, "triples.db"));
	...
	it("supports single-current-truth CRUD and historical queries", () => {
		const dbPath = nodePath.join(tempRoot(), "triples.db");
