...
function tempRoot(): string {
	const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-c25-delta-"));
	roots.push(root);
	return root;
}
...
function seededMemory(): { memory: Mnemosyne; root: string } {
	const root = tempRoot();
	const memory = new Mnemosyne({ sessionId: "s1", dbPath: nodePath.join(root, "mnemosyne.db") });
...
describe("C25 DeltaSync table allowlist", () => {
	...
	it("accepts allowed tables and rejects unknown, injected, and non-string table values", () => {
		...
		try {
			const sync = new DeltaSync(memory, nodePath.join(root, "sync"));
...
describe("C25 DeltaSync checkpoint compatibility", () => {
	it("scopes checkpoints by peer and table", () => {
		const { memory, root } = seededMemory();
		try {
			const dir = nodePath.join(root, "sync");
	...
	it("loads legacy per-peer checkpoint files as working-memory checkpoints", () => {
		...
		try {
			...
			fs.writeFileSync(
				nodePath.join(dir, "legacy-peer.json"),
				JSON.stringify({
					peer_id: "legacy-peer",
