...
function useTempBlobDir(): string {
	const dir = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-blobs-"));
	process.env.MNEMOSYNE_BLOB_DIR = nodePath.join(dir, "blobs");
...
describe("content sanitizer blob storage", () => {
	it("stores blobs by sha256 and is idempotent", () => {
		...
		const path = nodePath.join(blobRoot, sha.slice(0, 2), sha.slice(0, 4), sha);
		const mtime = fs.statSync(path).mtimeMs;

		expect(sha).toHaveLength(64);
		expect(computeSha256(data)).toBe(sha);
		expect(fs.readFileSync(path)).toEqual(data);
