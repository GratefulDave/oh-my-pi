...
beforeEach(() => {
	dataDir = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-ts-provider-parity-"));
...
describe("provider all-tools parity", () => {
	...
	it("exports provider data to a file and imports it into a fresh isolated bank", () => {
		...
		const exportPath = nodePath.join(dataDir, "provider-export.json");
		...
		const payload = JSON.parse(fs.readFileSync(exportPath, "utf8")) as { working_memory?: unknown[] };
