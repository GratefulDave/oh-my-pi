...
beforeEach(() => {
	dataDir = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-mcp-server-"));
...
describe("MCP JSON handlers", () => {
	...
	it("routes bank paths through BankManager validation and canonical layout", () => {
		...
		expect(defaultStats.db_path).toBe(nodePath.join(dataDir, "mnemosyne.db"));

		const workStats = handleToolCall("mnemosyne_diagnose", { bank: "work" });
		expect(workStats.db_path).toBe(nodePath.join(dataDir, "banks", "work", "mnemosyne.db"));
