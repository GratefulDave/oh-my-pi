...
function withDb<T>(fn: (path: string, cons: VeracityConsolidator) => T): T {
	const dir = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-veracity-"));
	const path = nodePath.join(dir, "facts.db");
...
describe("compute_fact_id", () => {
	it("is deterministic and uses the stable SHA-256 framed format", () => {
		...
		expect(id).toBe(`cf_${crypto.createHash("sha256").update(framed).digest("hex").slice(0, 24)}`);
	});
