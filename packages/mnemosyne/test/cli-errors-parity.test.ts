...
beforeEach(() => {
	root = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-ts-cli-errors-parity-"));
...
describe("CLI usage and operation failure parity", () => {
	...
	it("reports import file and JSON validation errors without tracebacks", async () => {
		...
		expect(await runCli(["import", nodePath.join(root, "missing-file.json")], missing.context())).toBe(1);
		...
		for (const payload of ["[]", '"not an export"']) {
			const path = nodePath.join(root, `not-object-${payload.length}.json`);
		...
		const badJson = nodePath.join(root, "bad.json");
	...
	it("export and import report actual memory counts", () => {
		...
		const exportPath = nodePath.join(root, "export.json");
		...
		expect(cmdImport([exportPath], importIo.context(nodePath.join(root, "imported")))).toBe(0);
