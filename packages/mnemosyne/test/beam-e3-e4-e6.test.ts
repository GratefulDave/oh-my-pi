...
function tempDb(name = "mnemosyne.db"): TempDb {
	const dir = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-beam-e3-e4-e6-"));
	const db = { dir, path: nodePath.join(dir, name) };
	tempDbs.push(db);
	return db;
}
...
afterEach(() => {
	...
	while (tempDbs.length > 0) {
		...
		if (db) fs.rmSync(db.dir, { recursive: true, force: true });
	}
});
...
describe("Beam E3/E4/E6 parity integration", () => {
	...
	it("auto-migrates legacy annotation triples once and writes a backup", () => {
		...
		try {
			...
			expect(fs.existsSync(`${db.path}.pre_e6_backup`)).toBe(true);
