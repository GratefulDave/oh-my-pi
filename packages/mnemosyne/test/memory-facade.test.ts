...
function tempRoot(): string {
	const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), "mnemosyne-memory-facade-"));
	roots.push(root);
	return root;
}
...
describe("Mnemosyne facade", () => {
	it("wraps BeamMemory for instance remember, recall, get, update, forget, stats, and context", () => {
		const dbPath = nodePath.join(tempRoot(), "mnemosyne.db");
	...
	it("stores duplicate-content batch items with distinct ids", () => {
		const memory = new Mnemosyne({ dbPath: nodePath.join(tempRoot(), "mnemosyne.db"), sessionId: "batch" });
		try {
	...
	it("preserves legacy and Python-compatible aliases", () => {
		const memory = new Mnemosyne({
			dbPath: nodePath.join(tempRoot(), "mnemosyne.db"),
			session_id: "aliases",
		});
		try {
