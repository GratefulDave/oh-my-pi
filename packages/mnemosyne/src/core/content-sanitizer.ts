...
export function blobRoot(env: NodeJS.ProcessEnv = process.env): string {
	return env.MNEMOSYNE_BLOB_DIR && env.MNEMOSYNE_BLOB_DIR.length > 0
		...
		: nodePath.join(os.homedir(), ".hermes", "mnemosyne", "blobs");
}
...
export function storeBlob(rawBytes: Uint8Array): string {
	...
	const blobDir = nodePath.join(blobRoot(), sha256.slice(0, 2), sha256.slice(0, 4));
	fs.mkdirSync(blobDir, { recursive: true });
	const blobPath = nodePath.join(blobDir, sha256);
	if (!fs.existsSync(blobPath)) fs.writeFileSync(blobPath, rawBytes);
	return sha256;
}
