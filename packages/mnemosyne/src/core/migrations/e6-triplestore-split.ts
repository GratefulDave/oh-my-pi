...
function copyDatabase(source: DatabasePath, destination: string): void {
	...
	try {
		...
		fs.writeFileSync(destination, (db as SerializableDatabase).serialize());
	} finally {
		closeQuietly(db);
	}
}
...
export function migrate(
	...
	if (effectiveBackup) {
		...
		if (fs.existsSync(backupPath)) effectiveLog(`Backup already exists at ${backupPath}; leaving as-is.`);
		else {
