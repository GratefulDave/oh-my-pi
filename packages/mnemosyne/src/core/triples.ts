...
export function legacyDataDir(env: ProcessEnv = process.env): string {
	return nodePath.join(homeDir(env), ".hermes", "mnemosyne", "data");
}
...
export function defaultTripleDbPath(env: ProcessEnv = process.env): string {
	return nodePath.join(defaultDataDir(env), "triples.db");
}

export function legacyTripleDbPath(env: ProcessEnv = process.env): string {
	return nodePath.join(legacyDataDir(env), "triples.db");
}
...
function copyLegacyDb(source: string, destination: string): void {
	...
	try {
		...
		fs.writeFileSync(tempPath, (sourceDb as SerializableDatabase).serialize());
		if (!fs.existsSync(destination)) fs.copyFileSync(tempPath, destination);
	} finally {
...
export function resolveDefaultTripleDb(env: ProcessEnv = process.env): string {
	...
	if (destination !== legacy && !fs.existsSync(destination) && fs.existsSync(legacy))
		copyLegacyDb(legacy, destination);
	return destination;
}
