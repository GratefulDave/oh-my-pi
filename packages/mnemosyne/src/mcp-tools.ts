...
function sharedBeam(): BeamMemory {
	...
	const dbPath =
		configured && configured.length > 0 ? configured : nodePath.join(dataDir(), "shared", DEFAULT_DB_FILENAME);
...
function handleImport(args: ToolArguments): ToolResult {
	...
	if (!fs.existsSync(inputPath)) return { error: `input_path does not exist: ${inputPath}` };
	return withBeam(args, (beam, bank) => {
