...
function resolveDbPath(context?: CliContext): string {
	return context?.dbPath ?? (context?.dataDir ? nodePath.join(context.dataDir, "mnemosyne.db") : configuredDbPath());
}
...
export const cmdImport: CommandHandler = (args, context) => {
	...
	if (!fs.existsSync(inputPath)) fail(`Import file not found: ${inputPath}`, 1);
	let parsed: unknown;
	try {
