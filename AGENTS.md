...
## Protected Fork Patches (never drop in an upstream sync)

Two fork-only patches are load-bearing and MUST survive every `upstream/main` merge. After each sync, run `bun "${LEX_MAINTENANCE_HOME:-../lex-maintenance}/scripts/check-fork-patches.ts" --repo "$PWD"` — both are checker-enforced (P11, P12). If a merge clobbers them, re-apply before rebuilding.

- **xai-oauth stream workarounds (P12)** — `packages/ai/src/providers/openai-responses.ts`, `openai-shared.ts`, `packages/ai/test/openai-responses-{stream-terminal,tool-quarantine}.test.ts`. Forces `parallel_tool_calls: true` for `xai-oauth` (api.x.ai defaults it off and honors only the first `function_call`), harvests terminal `function_call`s from `response.completed.output`, and ingests Completions-shaped `tool_calls` on a Responses stream, finalizing on frame-less close. Gated strictly on `xai-oauth` — never extend to other providers. Committed on the `upstream-18.0.4` lane (`d79533f878`); not upstreamed (rejected there).
- **Bash minimizer gain telemetry (P11)** — `packages/coding-agent/src/tools/bash-minimizer-gain.ts`, writers in `tools/bash.ts` / `session/bash-runner.ts`, `shellMinimizer.gainTelemetry` schema key (default **true**), stats gain dashboard. Capture is always on: the helper must `return true` with no disable path. The checker expects schema default `true` and rejects a fail-closed `value === true` gate — do not "fix" it to `false` or fail-closed. Upstream half lives in PR #5795 (open).

Do not close or abandon upstream PR #5795. The retired patches (P1 `replaceModelRoles`, P4 `metaColor`, P6 `disabled-agents`, P8 Codex routing, pm-profile extension) were dropped deliberately on 2026-08-25 — see `docs/upstream-rebase-and-fork-maintenance.md` and snapshot branch `pre-patch-retirement-20260825` if they are ever needed again.
