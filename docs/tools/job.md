...
- Async jobs also appear in the session observer as embedded cards when `AsyncJobManager` publishes lifecycle/progress metadata. Bash job cards show job status and recent progress text; task job cards also include task `AgentProgress` when the task executor reports it. Task-job observer updates are correlated by async job id and `runMetadata.runId`, not by the original task item id. `job` remains the control surface for polling, cancelling, listing, and acknowledging async job delivery.
- Native async job cards do not spawn tmux/cmux panes or windows. Pane/window details may appear only as presentation metadata from a pane/window-capable backend or plugin. The observer transcript view appears only when the observed run has a readable `sessionFile`. Transcript artifacts are surfaced as artifact refs on metadata-only cards; they are not parsed as transcript view unless a run also attaches them as the session file.
