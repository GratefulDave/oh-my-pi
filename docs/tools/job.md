...
- Async jobs appear in the TUI as inline `● Agents` cards when `AsyncJobManager` publishes lifecycle/progress metadata. Bash job cards show job status and recent progress text; task job cards also include task `AgentProgress` and run metadata when the task executor reports it. `job` remains the control surface for polling, cancelling, listing, and acknowledging async job delivery.
- Native async job cards do not spawn tmux/cmux panes or windows. Visible pane/window details appear only when the cmux/pi-subagents spawn/event path supplies real presentation metadata (`mode`, `backend=cmux`, `session`, `paneId`, `windowId`). The observer transcript view appears only when the observed run has a readable `sessionFile`; metadata-only cards still display run state, artifacts, and presentation fields.
- The observer overlay (modal detail view) is opened manually via the session-observer keyboard shortcut; it does not open automatically when jobs start. Enter expands the selected card entry. Terminal mouse click toggles an entry, but **terminal mouse is off by default** — enable with `PI_TUI_MOUSE=1` to opt in. Without that env var, click events are not captured so normal text selection works.
...
- User-visible prompts / interactive UI
  ...
  - Async jobs may be visible in the session observer through embedded cards managed by `SessionObserverRegistry`; the observer does not replace `job` polling/cancellation semantics or imply a visible pane/window backend.
